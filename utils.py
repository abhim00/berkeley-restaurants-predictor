...
def map_and_filter(s, map_fn, filter_fn):
    ...
    """
    # BEGIN Question 0
    return [map_fn(var) for var in s if filter_fn(var)]    # END Question 0
...
def enumerate(s, start=0):
    ...
    # BEGIN Question 0
    temp = []
    for var in range(start, start+len(s)):
        temp += [var]
    return zip(temp, list(s))
    # END Question 0
...
def mean(s):
    ...
    """
    # BEGIN Question 1
    assert len(s)>0, 'That list needs more values'
    total = 0
    for val in s:
        total+=val
